...
public class CreatePolygonFromSprite
{
    ...
    public GameObject Run (Transform transform,bool ReverseNormals, int triangleIndex)
	{
		...
		if(AssetDatabase.LoadAssetAtPath(_puppet2DPath+"/Models/"+transform.name+"_MESH.asset",typeof(Mesh)))
		{
			...
			{
				//mf.mesh = AssetDatabase.LoadAssetAtPath(_puppet2DPath+"/Models/"+transform.name+"_MESH.asset",typeof(Mesh))as Mesh;
				string meshPath = (_puppet2DPath+"/Models/"+transform.name+"_MESH.asset");
				AssetDatabase.CreateAsset(mesh,meshPath);
				overwrite = true;
			}
			else
			{
				string meshPath = AssetDatabase.GenerateUniqueAssetPath(_puppet2DPath+"/Models/"+transform.name+"_MESH.asset");
				AssetDatabase.CreateAsset(mesh,meshPath);
			}
		}
		else
		...
		if(overwrite)
		{
			mr.material = AssetDatabase.LoadAssetAtPath(_puppet2DPath+"/Models/Materials/"+transform.name+"_MAT.mat",typeof(Material)) as Material;
		}
		else
		{

			Material newMat = new Material(Shader.Find("Unlit/Transparent"));
			string materialPath = AssetDatabase.GenerateUniqueAssetPath(_puppet2DPath+"/Models/Materials/"+transform.name+"_MAT.mat");
    ...
    public static void PolyGenForSprite(Sprite sprite)
    {
    ...
    public GameObject MakeFromVerts (bool ReverseNormals ,Vector3[] vertsToCopy, List<int> pathSplitIds, GameObject FFDGameObject)
    {
        ...
        if(AssetDatabase.LoadAssetAtPath(_puppet2DPath+"/Models/"+FFDGameObject.transform.name+"_MESH.asset",typeof(Mesh)))
        {
            if(EditorUtility.DisplayDialog("Overwrite Asset?","Do you want to overwrite the current Mesh & Material?","Yes, Overwrite","No, Create New Mesh & Material"))
            {
                //mf.mesh = AssetDatabase.LoadAssetAtPath(_puppet2DPath+"/Models/"+transform.name+"_MESH.asset",typeof(Mesh))as Mesh;
                string meshPath = (_puppet2DPath+"/Models/"+FFDGameObject.transform.name+"_MESH.asset");
                AssetDatabase.CreateAsset(mesh,meshPath);
                overwrite = true;
            }
            else
            {
                string meshPath = AssetDatabase.GenerateUniqueAssetPath(_puppet2DPath+"/Models/"+FFDGameObject.transform.name+"_MESH.asset");
                AssetDatabase.CreateAsset(mesh,meshPath);
            }
        }
        else
        ...
        if(overwrite)
        {
            mr.material = AssetDatabase.LoadAssetAtPath(_puppet2DPath+"/Models/Materials/"+FFDGameObject.transform.name+"_MAT.mat",typeof(Material)) as Material;
        }
        else
        {

            Material newMat = new Material(Shader.Find("Unlit/Transparent"));
            string materialPath = AssetDatabase.GenerateUniqueAssetPath(_puppet2DPath+"/Models/Materials/"+FFDGameObject.transform.name+"_MAT.mat");
            AssetDatabase.CreateAsset(newMat, materialPath);
            mr.material = newMat;
        }
